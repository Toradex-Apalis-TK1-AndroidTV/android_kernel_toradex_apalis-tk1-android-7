...
static void apalis_tk1_hdmi_hotplug_report(bool state)
{
...
static struct tegra_dc_out apalis_tk1_disp2_out = {
	...
	.hotplug_report = apalis_tk1_hdmi_hotplug_report,
};
...
static void apalis_tk1_panel_select(void)
{
	...
	if (panel) {
		...
		if (panel->init_dc_out) {
			...
			apalis_tk1_disp1_out.dsi->dsi_instance = dsi_instance;
			apalis_tk1_disp1_out.dsi->dsi_panel_rst_gpio =
				DSI_PANEL_RST_GPIO;
			apalis_tk1_disp1_out.dsi->dsi_panel_bl_pwm_gpio =
				DSI_PANEL_BL_PWM_GPIO;
			tegra_dsi_update_init_cmd_gpio_rst(&apalis_tk1_disp1_out);
		}
...
int __init apalis_tk1_panel_init(int board_id)
{
	int err = 0;
	...
	if ((bi.sku == BOARD_SKU_FOSTER) && (bi.board_id == BOARD_P2530)) {
		...
		apalis_tk1_disp2_fb_data.xres = 1920;
		apalis_tk1_disp2_fb_data.yres = 1080;
		apalis_tk1_disp2_device.id = 0;
		apalis_tk1_disp2_out.parent_clk = "pll_d";

		apalis_tk1_disp2_out.modes = hdmi_panel_modes;
		apalis_tk1_disp2_out.n_modes = ARRAY_SIZE(hdmi_panel_modes);
	} else
	...
	apalis_tk1_disp2_out.hdmi_out = &apalis_tk1_hdmi_out;
